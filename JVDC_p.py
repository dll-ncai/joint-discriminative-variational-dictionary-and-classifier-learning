...
#f.write("\nSummary of "+ str(rcounter) + " experiments of YaleB dataset for face recognition\n")
...
def extended_yaleB(features,labels,counter):
    ...
    for c in range(classes):
        ...
        #trainColIndex = trainColIndex + rd.sample(classIndex, 32)
...
def caltech_data(features,labels,counter):
    ...
    test_label=labels[:,testColIndex]
    return training_samples, train_label, test, test_label
def VI_main(named,iiii,dicSize,DB,Code_DPath,DBdisc,k0,mi,gibi,sparsity,a0,b0,c0,d0,e0,f0,g_eps_d,g_eps_w,g_sd,g_sw,stp, batchsize,trExPerClass,foperation="w+"):
    if named==1:
        ...
        fh=0
        while counter!=10:
            f = open(Code_DPath+"JVDC_p_results_YaleB.txt", foperation)
            strn = ""
            training_samples, train_label, test, test_label  =extended_yaleB(features, labels, counter)
            algosettings, iter, oiter,accuracyi,accuracy, trainTime, trainRateTime, testRateTime, initDictSize, finalDictSize = JVDC_p(stp, batchsize,trExPerClass,DBdisc,dicSize,iiii,training_samples, train_label, test,test_label,k0,mi,gibi,sparsity,a0,b0,c0,d0,e0,f0,g_eps_d,g_eps_w,g_sd,g_sw)
            ...
            #strn = "\n===Experiment No. " + str(rcounter) + " of YaleB dataset for face recognition===\nmean accuracy = " +str(smean)+ "\n"+algosettings+"\n"
            if fh==0:
                f.write("\n\n\n###Experiment for YaleB dataset for face recogition ("+str(datetime.now())+")###\n")
                fh=1
            #f.write("======================Experiment No. " + str(rcounter) + " of YaleB dataset for face recognition===============\n")
    ...
    if named==2:
        ...
        while gcounter<6:
            ...
            fh=0
            while counter!=1:
                f = open(Code_DPath+"JVDC_p_results_caltech.txt", foperation)
                strn = ""
                ...
                #strn = "===Experiment No. " + str(rcounter) + " for training samples " + str(counter * 5) + " per class of Caltech database for object classification===\nmean accuracy = " +str(smean)+"\n"+ algosettings+"\n"
                if fh==0:
...
def initDSeqSelection(train, label, k):
    ...
    # normalized the colums in l_2 to take advantage of Cholesky based implementation of OMP for initialization.
    return Dinit, label
...
def params_intitializer_gibb_sampling(training_samples, train_label, Dinit, Winit, Alpha_init, Beta_init, pars,gibi,sparsity,g_eps_d,g_eps_w,g_sdd,g_sww,batchsize,pruneDict, Xdclist,classItemsCount):
    ...
    for iter in range(DLiterations):  #
        if iter==DLiterations-1:
            ...
            L_s_a, L_s_b= sample_g_s_to_VI_Input(train_label, Sd,c0, d0, Z)
            pars.update(
                {'elpha': elpha1.T, 'beta': beta1.T,'L_y_a':L_y_a, 'L_y_b':L_y_b,'L_h_a':L_h_a,'L_h_b':L_h_b,'L_s_a':L_s_a, 'L_s_b':L_s_b,
...
def VI_D_W_Learning(Xd,D,Dv,Hd,W,Wv,Sd,Sdv,Z,batchsizee,L_y_a, L_y_b, L_h_a, L_h_b): # last ok editing
    K = D.shape[1]
    L_y=-(L_y_a+1)/L_y_b
    L_h=-(L_h_a+1)/L_h_b
    fl=0
...
def VI_Pi_Learning(train_label,Z,a0,b0,classesItemsCount):
    K = Z.shape[0];
    elpha=(Z.dot(train_label.T))+ a0 / K
    beta=(classesItemsCount-(Z.dot(train_label.T)))+ b0 * (K - 1) / K
    Pics=np.random.beta(elpha,beta)     
    return Pics,elpha,beta
def VI_Ly_Lh_Learning(Xd,D,Dv,Sd,Sdv,Hd,W,Wv,Z,e0,f0):
    ...
    a2=0.5*np.sum((a+b-c))+f0;
    a1=(0.5*Xd.shape[1]*Xd.shape[0]+e0-1)
    L_y_a=a1-1
    L_y_b=-a2
    ...
    a2 = 0.5*np.sum((a + b - c)) + f0;
    a1 = (0.5*Hd.shape[1] * Hd.shape[0] + e0-1)
    L_h_a=a1-1
    L_h_b=-a2
    return L_y_a,L_y_b,L_h_a,L_h_b
def VI_Ls_Learning(labels,Sd,Sdv,c0,d0,classItemsCount): 
    a1=(c0+0.5*((classItemsCount).T)*Sdv.shape[0])
    a2=(0.5*(np.sum(labels.dot(((-0.5*Sd/Sdv)**2+(-0.5/Sdv)).T),1)[:,np.newaxis])+d0)
    L_s_a=a1-1
    L_s_b=-a2
    return L_s_a,L_s_b

def JVDC_p(stp, batchsizee,trExPerClass, DBdisc,dicSize,iiii,training_samplesm, train_labelm, test, test_label,k0,mi,gibi,sparsity,a0,b0,c0,d0,e0,f0,g_eps_dd,g_eps_ww,g_sdd,g_sww):
    starttime = time.time()
    ...
    for ii in range(lenl):
        ...
        while t<lpcnt:
            ...
            Xd,Hd,D, W, Dv,Wv = VI_D_W_Learning(Xd, D,Dv,Hd, W, Wv,Sd, Sdv, Z,
                                                                    batchsize,L_y_a, L_y_b, L_h_a, L_h_b)
            L_y_a,L_y_b,L_h_a,L_h_b = VI_Ly_Lh_Learning(Xd, D, Dv, Sd, Sdv,
                                                          Hd, W, Wv, Z, e0, f0)
            Pics,elpha,beta = VI_Pi_Learning(train_label,Z, a0, b0,classItemsCount);
                
            L_s_a,L_s_b= VI_Ls_Learning(train_label,Sd, Sdv, c0, d0,classItemsCount)
            ...
            if t!=0:
                obj=obj+np.sum((Zv-Zp)*(1.0/(1+np.exp(-Zp)))+Zp) #+np.log(1+np.exp(-Zp))) #np.log(1+np.exp(Zp)))
                ypnts=ypnts+[obj]
                xpnts=xpnts+[t]
            ######################################################################################3
            if t%5==0:
                ...
                print(f"Increase in ELBO Value  = {(obj-previous):8.2e}")
            previous=obj
    ...
    algosettings = "Date and time = " + str(datetime.now()) + ", k0 = " + str(k0) + ", a0 = " + str(a0) + ", b0 = " + str(b0) + ", c0 = " + str(c0) + ", d0 = " + str(d0) + ", e0 = " + str(e0) + ", f0 = " + str(f0) + ", sparsity = " + str(sparsity) + ", Iterations  = " + str(t+1) + ", train examples = " + str(trnexamples)
...
while f<2:
    f=f+2
    if f==1:
        DBdisc="JVDC_p training on Extended Yale database for face recognition"
        k0=1e-6 #(Threshold for pruning of dictionary atoms)
        mi = 500 #(Number of iterations)
        gibi=5    #(Number of iterations of Gibbs Sampling for initilization of this algorithm--One iteration means without initilizing using Gibbs Sampling)--Variational inference works if initialized with few iterations of Gibbs Sampling.
        sparsity=30
        #Hyper parameters as explained in the paper
        a0=0 # (Value assigned in the algorithm)
        b0=0 # (Value assigned in the algorithm)
        c0=1.e-6
        d0=1.e-6
        e0=1.e-6
        f0=1.e-6
        ...
        batchsize=1 # Number of dictionary atoms to be trained in one iteration (0 for full dictionary atoms)
                    #Variational inference works only by setting it equal to 1 because it gets stuck at local maxima and does not get escape as Stochastic variational inference does, in case of training multiple atoms in one go, because of noisy natural gradient.
        pruneDict=1 #(1 if pruning allowed, 0 if not)--We can perform experiments with pruning or without pruning, depending upon dictionary auto setting
        dicSize=0 # Dictionary Size (0 if auto selection)--We can perform by fixing dictionary atoms or with auto option
        q=VI_main(1,pruneDict,dicSize,'./ExtendedYaleB.mat',Code_DPath,DBdisc,k0,mi,gibi,sparsity,a0,b0,c0,d0,e0,f0,g_eps_d,g_eps_w,g_sd,g_sw,stp,batchsize,0,foperation="a+") # 1- for Yale database

    if f==2:
        DBdisc="JVDC_p training on AR database for face recognition"
        k0=1e-6 #(Threshold for pruning of dictionary atoms)
        mi = 500 #(Number of iterations)
        gibi=5    #(Number of iterations of Gibbs Sampling for initilization of this algorithm--One iteration means without initilizing using Gibbs Sampling)--Variational inference works if initialized with few iterations of Gibbs Sampling.
        sparsity=80
        ...
        stp=0 # Number of Classifiers to be learned for voting (0 or 1 for one classifier)--Extra option
        batchsize=1 # Number of dictionary atoms to be trained in one iteration (0 for full dictionary atoms)
                    #Variational inference works only by setting it equal to 1 because it gets stuck at local maxima. In contrary to VI, Stochastic variational inference (SVI) gets escape, in case of training multiple atoms in one go, because of noisy natural gradient.
        pruneDict=1 #(1 if pruning allowed, 0 if not)--We can perform experiments with pruning or without pruning, depending upon dictionary auto setting
        dicSize=0# Dictionary Size (0 if auto selection)--We can perform by fixing dictionary atoms or with auto option
        q=VI_main(2,pruneDict,dicSize,'./caltechData.npz',Code_DPath,DBdisc,k0,mi,gibi,sparsity,a0,b0,c0,d0,e0,f0,g_eps_d,g_eps_w,g_sd,g_sw,stp,batchsize,0,foperation="a+")
